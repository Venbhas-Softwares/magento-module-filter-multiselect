 ...
 * Expands layered-navigation sections that contain the active filter after full page load.
 *
 * Hyvä: options sit inside <template x-if="open">; filter cards often use x-defer="intersect" so
 * Alpine does not bind until the card is in view — $data()/clicks do nothing until then. We wait for
 * intersection, then set open (Alpine proxy: use 'open' in d, not only hasOwnProperty).
 */
(function () {
    ...
    /**
     * Request-var keys present in the URL with a non-empty value (e.g. style, style[] → style).
     * Used to open the correct layered-nav section when chip labels do not match card titles (common for Style).
     */
    ...
    /**
     * Visible title text for a Hyvä filter-option card (theme variants differ).
     * Hyvä default: <button class="filter-options-title ..."><span class="title ...">Style<span class="sr-only"> filter</span></span>
     */
    ...
    /**
     * Element that holds Alpine { open } or native <details> (Hyvä keeps filter HTML inside x-if="open",
     * so the option list is not in the DOM until open — we must toggle this root, not rely on ol markers).
     */
    ...
    function isHyvaFilterExpanded(card) {
        ...
        var ae = b.getAttribute('aria-expanded');
        return ae === 'true';
    }
    ...
    /**
     * Match URL request param to storefront filter heading (e.g. style_bags vs label "Style" → style).
     * Magento often uses request vars like style_bags while the visible title is only "Style".
     */
    function requestVarMatchesHyvaCardTitle(base, cardTitle) {
        ...
        if (cardTitle.length >= 2 && b.indexOf(cardTitle + '_') === 0) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Match URL request var (e.g. style, style_bags, product_size) to visible filter title when ol is not in DOM yet.
     */
    function cardMatchesUrlParamToTitle(card, urlVars) {
        ...
        return list.some(function (base) {
            ...
            return requestVarMatchesHyvaCardTitle(base, cardTitle);
        });
    }

    /**
     * Hyvä x-defer="intersect": Alpine binds only after the card intersects the viewport.
     * Wait for intersection, then poll $data / click — otherwise toggles never apply.
     */
        ...
        function beginOpenAttempts() {
            ...
            function tick() {
                ...
                if (toggleRoot) {
                    ...
                    if (openedViaAlpineData && typeof window.Alpine !== 'undefined' && typeof window.Alpine.nextTick === 'function') {
                        try {
                ...
                if (allowClickFallback && attempt < maxAttempts) {
                    ...
                    if (btnFallback) {
                        ...
                        var aeFb = btnFallback.getAttribute('aria-expanded');
                        if (alpineClosed || aeFb === 'false') {
                            card.dataset.venbhasHyvaClickDone = '1';
                            btnFallback.click();
                        }
                    }
    ...
    /**
     * Open Luma accordion / Hyvä ancestors for lists whose request var is in the URL (Luma + Hyvä).
     */
    ...
    /**
     * Hyvä layered nav: open .filter-option cards that match "Active filtering" chip labels.
     * x-defer="intersect" means Alpine may not expose $data() until the card has been in view;
     * clicks before init do nothing. We scroll into view, poll $data().open, then click once as fallback.
     */
    function expandHyvaLayeredNavigationPanels() {
        ...
        function cardMatchesLabel(card) {
            ...
            return labels.some(function (lb) {
                return (
                    cardTitle === lb ||
                    cardTitle.indexOf(lb) === 0 ||
                    lb.indexOf(cardTitle) === 0
                );
            });
        }
        ...
        document.querySelectorAll('.filter-option').forEach(function (card) {
            if (
                cardMatchesLabel(card) ||
                cardMatchesRequestVar(card) ||
                cardMatchesUrlParamToTitle(card, urlVars)
            ) {
    ...
    /**
     * Open Alpine.js x-data sections (Hyvä / generic) when possible.
     */
    ...
    /**
     * Luma accordion / collapsible: show content and mark section active.
     */
    ...
    function expandFromNode(start) {
        ...
        alpineOpenAncestors(start);
    }

    /**
     * Swatch filters — attribute code class on URL param (e.g. style_bags).
     */
    ...
    function expandActiveFilterSections() {
        ...
        document.querySelectorAll('.filter-options-item._filter-active').forEach(function (item) {
            lumaOpenSection(item);
        });
